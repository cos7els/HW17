...
public class DocsWorker {
    ...
    private void readData(File file) {
        ...
        for (String s : strings) {
            DOC_NUMBER_PATTERN.matcher(s).results().forEach(x -> document.addDocNumbers(x.group()));
            PHONE_NUMBER_PATTERN.matcher(s).results().forEach(x -> document.addPhoneNumbers(x.group()));
            EMAIL_PATTERN.matcher(s).results().forEach(x -> document.addeMails(x.group()));
        }
